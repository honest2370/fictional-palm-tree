...
// Seller-only payout request. Debits the wallet immediately and logs a
// pending withdrawal for an admin to action manually (Ashtech's Direct
// collect API is inbound-only; payouts are settled off-platform by the
// operator, same as the original app).
...
Deno.serve(async (req) => {
  ...
  const { data: user } = await admin.from("users").select("balance,name,role,currency").eq("id", userId).maybeSingle();
  ...
  const insertPayload: Record<string, unknown> = {
    user_id: userId, user_name: user.name, amount: withdrawAmount, currency: user.currency, method, status: "pending",
  };
  ...
  if (insErr) {
    await admin.from("users").update({ balance: user.balance }).eq("id", userId); // roll back
