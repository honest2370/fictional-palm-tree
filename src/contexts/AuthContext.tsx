...
interface AuthContextValue {
  user: AppUser | null;
  loading: boolean;
  signUp: (email: string, password: string, role: UserRole, extra: { name: string; phone: string; country_code: string; currency: string; storeName?: string }) => Promise<{ error: string | null }>;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  async function signUp(
    ...
    extra: { name: string; phone: string; country_code: string; currency: string; storeName?: string },
  ) {
    ...
    const storeName = extra.storeName?.trim() || extra.name;
    if (role === "seller") {
    ...
    const { error: insertErr } = await sb.from("users").insert({
      ...
      ...(storeSlug ? { store_name: storeName, store_slug: storeSlug } : {}),
    });
  ...
  async function generateUniqueStoreSlug(name: string): Promise<string> {
    ...
    let candidate = base;
    for (let i = 0; i < 5; i++) {
      const { data } = await sb.from("users").select("id").eq("store_slug", candidate).maybeSingle();
