...
export default function MyCourses() {
  ...
  const [active, setActive] = useState<CourseEntry | null>(null);
  ...
  useEffect(() => {
    if (!user) return;
    sb.from("order_items")
      .select("id, product:product_id(*), order:order_id!inner(status,buyer_id)")
      .eq("order.buyer_id", user.id).eq("order.status", "confirmed")
      .then(({ data }) => {
        const courseItems = ((data as any[]) || []).filter((row) => row.product?.type === "course");
