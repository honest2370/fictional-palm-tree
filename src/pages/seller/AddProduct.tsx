...
import type { ProductType, CourseModule, CourseLesson, Category } from "@/types";
...
export default function AddProduct() {
  ...
  useEffect(() => {
    sb.from("categories").select("*").eq("is_active", true).order("sort_order").then(({ data }) => setCategories((data as Category[]) || []));
  }, []);
  ...
  async function handleSubmit() {
    ...
    const { data: inserted, error } = await sb.from("products").insert({
      seller_id: user.id, seller_name: user.name, title, slug: slugifyUnique(title), description, price: Number(price),
      type, delivery_link: type !== "account" ? deliveryLink : null, cover_url: coverUrl || null,
      status: "pending", curriculum, category_id: categoryId || null,
  ...
  return (
    <div className="p-4 pb-28 bg-slate-950 min-h-screen">
      ...
      <label className={labelClass}>SEO title (optional — shown in Google search results)</label>
      <input className={fieldClass} value={seoTitle} onChange={(e) => setSeoTitle(e.target.value)} placeholder="Defaults to product title" />

      <label className={labelClass}>SEO description (optional)</label>
      <textarea className={fieldClass} rows={2} value={seoDescription} onChange={(e) => setSeoDescription(e.target.value)} placeholder="Short summary for search engines (155 chars)" />
