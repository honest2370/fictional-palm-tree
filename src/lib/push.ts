...
export function usePushNotifications() {
  ...
  const subscribe = useCallback(async () => {
    ...
    try {
      ...
      // applicationServerKey expects BufferSource | string. Uint8Array's
      // .buffer is typed as ArrayBufferLike (ArrayBuffer | SharedArrayBuffer)
      // which TS won't narrow automatically — slice() returns a plain
      // ArrayBuffer-backed view, satisfying the DOM lib's stricter type.
      const applicationServerKey = urlBase64ToUint8Array(VAPID_PUBLIC_KEY).slice().buffer;
